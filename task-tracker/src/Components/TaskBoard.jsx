...
export default function TaskBoard({ tasks, onMove, onDelete }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3  gap-6 mt-10 ">
      {STATUSES.map(status => (
        <section key={status}
          className="bg-white rounded-xl shadow-lg p-4 min-h-[250px] flex flex-col border-2 border-blue-100 mt-8">
          <h2 className="text-xl font-bold text-gray-700 mb-3 text-center uppercase">{status}</h2>
          <div className="flex-1 flex flex-col gap-3">
            {tasks.filter(t => t.status === status).length === 0 ? (
              <div className="text-gray-500 text-lg text-center my-auto">No tasks</div>
            ) : (
              tasks
 ...
 >
                    ...
                    <div className="flex gap-2 mt-auto self-end mb-6">
                      {/* Only show "Move Forward" if not in final status */}
                      {task.status !== "Done" && (
                        <button
                          className="bg-green-500 text-white rounded px-3 py-1 hover:bg-green-600 text-m font-medium"
                      ...
                      <button
                        className="bg-red-500 text-white rounded px-3 py-1 hover:bg-red-600 text-m font-medium"
